...
export class ShiftFormModalComponent {
    ...
    onSave(): void {
        ...
        if (this.maxNurses < this.minNurses) {
            this.errorMessage = 'Maximum nurses must be less than minimum nurses';
            return;
        }
        if (this.minRAs < 0) {
        ...
        if (this.maxRAs < this.minRAs) {
            this.errorMessage = 'Maximum RAs must be less than minimum RAs';
            return;
        }
        if (this.maxTotalStaff < this.minNurses + this.minRAs) {
            this.errorMessage =
                'Maximum total staff must be more than minimum nurses + minimum RAs';
            return;
        }
        if (this.maxTotalStaff > this.maxNurses + this.maxRAs) {
            this.errorMessage =
                'Maximum total staff must be less than maximum nurses + maximum RAs';
            return;
        }
